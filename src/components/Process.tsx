...
const Process = () => {
  ...
  return <section id="process" className="section-padding bg-background">
      <div className="container px-4 md:px-8 mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={toggleEditMode} className="flex items-center gap-2 px-4 py-2 bg-secondary/10 rounded-md text-secondary hover:bg-secondary/20 transition-colors">
            {isEditing ? <>
        ...
        <div className="space-y-20 md:space-y-24">
          {steps.map((step, index) => <AnimateInView key={step.number} animation="fade-in" delay={index * 100}>
              <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-12 items-center", index % 2 === 1 ? "md:grid-flow-dense" : "")}>
                ...
                <div className={index % 2 === 1 ? "md:col-start-1" : ""}>
                  {isEditing ? <div className="relative">
                      <div className="mb-2 p-2 bg-gray-50 border rounded-md flex justify-end">
                        <select value={objectFitSettings[step.number] || "cover"} onChange={e => handleObjectFitChange(step.number, e.target.value as any)} className="text-xs px-2 py-1 rounded border">
                      ...
                      <ImageSelector value={localImages[step.number] || step.image} onChange={url => handleImageChange(step.number, url)} aspectRatio={4 / 3} placeholder={`Select image for ${step.title}`} objectFit={objectFitSettings[step.number] || "cover"} />
                      ...
                      <img src={localImages[step.number] || step.image} alt={`Step ${step.number}: ${step.title}`} loading="lazy" className="object-cover" />
                      <div className="absolute -bottom-4 -right-4 bg-background p-2 px-4 rounded-full shadow-lg border border-border/50 text-sm font-medium">
                        {step.title}
                      </div>
                    </div>}
        ...
        <AnimateInView animation="fade-in" delay={300} className="mt-20">
          <div className="bg-secondary/20 rounded-2xl p-8 md:p-12 border border-border/20">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              {materials.map((material, index) => <AnimateInView key={material.id} animation="slide-in-up" delay={index * 200}>
                  <div className="bg-background rounded-xl p-6 border border-border/50 h-full hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
                    {isEditing ? <div className="space-y-4">
                        <div className="mb-2 p-2 bg-gray-50 border rounded-md flex justify-end">
                          <select value={materialObjectFitSettings[material.id] || "cover"} onChange={e => handleMaterialObjectFitChange(material.id, e.target.value as any)} className="text-xs px-2 py-1 rounded border">
                            <option value="contain">Contain</option>
                            <option value="cover">Cover</option>
                            <option value="fill">Fill</option>
                            <option value="scale-down">Scale Down</option>
                        ...
                        <ImageSelector value={materialLocalImages[material.id] || material.image} onChange={url => handleMaterialImageChange(material.id, url)} aspectRatio={1} placeholder={`Select image for ${material.title}`} objectFit={materialObjectFitSettings[material.id] || "cover"} maxHeight={100} />
                        
                        <div className="mt-4">
                          <label className="block text-sm font-medium mb-2">Icon Image (Optional)</label>
                          <ImageSelector value={materialIconImages[material.id]} onChange={url => handleMaterialIconChange(material.id, url)} aspectRatio={1} placeholder={`Select icon for ${material.title}`} objectFit="cover" maxHeight={80} />
                        </div>
                      </div> : <div className="h-12 w-12 flex items-center justify-center rounded-full bg-primary/10 text-primary mb-4">
                        {materialIconImages[material.id] ? <img src={materialIconImages[material.id]} alt={`${material.title} icon`} className="w-full h-full object-cover rounded-full" /> : materialLocalImages[material.id] ? <img src={materialLocalImages[material.id]} alt={material.title} className={`w-full h-full object-${materialObjectFitSettings[material.id] || "cover"} rounded-full`} /> : <img src={material.image} alt={material.title} className="w-8 h-8 object-cover rounded-full" />}
                      </div>}
