...
const HeroContent = ({
  specifications,
  catalogPdfUrl = "1lCoKIPFn63So99eKhnYt8w49DXo9_UAa"
}: HeroContentProps) => {
  return <div className="lg:col-span-6 lg:col-start-1 lg:pr-8 max-w-2xl">
      <AnimateInView animation="fade-in" delay={300}>
        <span className="bg-primary/20 text-white tracking-wider uppercase font-semibold px-3 py-1 rounded-full inline-block mb-4 text-sm">
      ...
      <AnimateInView animation="slide-in-up" delay={700}>
        <div className="bg-black/50 backdrop-blur-sm rounded-lg p-4 border border-white/20 mb-6 max-w-md">
          ...
          <ul className="space-y-2">
            {specifications.map((spec, index) => <li key={index} className="flex items-start">
      ...
      <AnimateInView animation="slide-in-up" delay={800}>
        <div className="flex flex-wrap gap-4">
          <a href="#products" className={cn("bg-primary text-primary-foreground hover:bg-primary/90", "px-5 py-2.5 rounded-md font-medium inline-flex items-center", "transition-all duration-300 transform hover:translate-y-[-2px]", "shadow-md")}>
            Explore Products
          </a>
          <DownloadCatalogButton variant="white" catalogId={catalogPdfUrl} />
        </div>
      </AnimateInView>
    </div>;
};
